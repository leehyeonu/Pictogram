...
def get_args():
    ...
    parser.add_argument("--height", help='cap height', type=int, default=3000)
...
def main():
    ...
    while True:
        ...
        #출력
        cv.namedWindow('Pictogram', cv.WND_PROP_FULLSCREEN)
        cv.setWindowProperty('Pictogram', cv.WND_PROP_FULLSCREEN, cv.WINDOW_FULLSCREEN)
        cv.namedWindow('Camera', cv.WND_PROP_FULLSCREEN)
        cv.setWindowProperty('Camera', cv.WND_PROP_FULLSCREEN, cv.WINDOW_FULLSCREEN)
        cv.imshow('Camera', debug_image01)
        cv.imshow('Pictogram', debug_image02)

        #노트북 출력
        cv.imshow('Pictogram - Admin', debug_image02)
        cv.imshow('Camera - Admin', debug_image01)
